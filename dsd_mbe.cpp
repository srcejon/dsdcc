...
void DSDMBEDecoder::processFrame(char imbe_fr[8][23], char ambe_fr[4][24], char imbe7100_fr[7][24])
{
    ...
    {
#ifdef DSDCC_MBELIB_NEO
        status = mbe_processImbe7200x4400Framef(m_audio_out_temp_buf, &result,
    ...
    {
#ifdef DSDCC_MBELIB_NEO
        status = mbe_processImbe7100x4400Framef(m_audio_out_temp_buf, &result,
                imbe7100_fr, imbe_d, m_mbelibParms->m_cur_mp,
                m_mbelibParms->m_prev_mp, m_mbelibParms->m_prev_mp_enhanced);
#else
    ...
    {
#ifdef DSDCC_MBELIB_NEO
        status = mbe_processAmbe3600x2400Framef(m_audio_out_temp_buf, &result,
    ...
    {
#ifdef DSDCC_MBELIB_NEO
        status = mbe_processAmbe3600x2450Framef(m_audio_out_temp_buf, &result,
                ambe_fr, ambe_d, m_mbelibParms->m_cur_mp,
                m_mbelibParms->m_prev_mp, m_mbelibParms->m_prev_mp_enhanced);
#else
    ...
    if (status < 0)
    {
        m_dsdDecoder->getLogger().log("mbelib-neo rejected MBE frame (%d)\n", status);
        return;
    }
    m_errs = result.c0_errors;
    m_errs2 = result.total_errors;
    mbe_formatProcessResult(m_err_str, sizeof(m_err_str), &result);
#endif
...
void DSDMBEDecoder::processData(char imbe_data[88], char ambe_data[49])
{
    ...
    {
#ifdef DSDCC_MBELIB_NEO
        status = mbe_processImbe4400Dataf(m_audio_out_temp_buf, &result,
    ...
    {
#ifdef DSDCC_MBELIB_NEO
        status = mbe_processAmbe2400Dataf(m_audio_out_temp_buf, &result,
    ...
    {
#ifdef DSDCC_MBELIB_NEO
        status = mbe_processAmbe2450Dataf(m_audio_out_temp_buf, &result,
                ambe_data, m_mbelibParms->m_cur_mp,
                m_mbelibParms->m_prev_mp, m_mbelibParms->m_prev_mp_enhanced);
#else
    ...
    if (status < 0)
    {
        m_dsdDecoder->getLogger().log("mbelib-neo rejected MBE data (%d)\n", status);
        return;
    }
    m_errs = result.c0_errors;
    m_errs2 = result.total_errors;
